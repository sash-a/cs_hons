// Taken from https://github.com/shao-xy/mpi-psrs/blob/master/PSRS.c
...
void phase3(int *array, int startIndex, int *partitionSizes, int **newPartitions, int *newPartitionSizes, int p)
{
    ...
    *newPartitions = (int *) malloc(totalSize * sizeof(int));
    ...
    //发送数据，实现n次点对点通信
    MPI_Alltoallv(&(array[startIndex]), partitionSizes, sendDisp, MPI_INT, *newPartitions, newPartitionSizes, recvDisp,
